...
import MovieCardExtended from './Components/MovieCardExtended';
import {
...
function App() {
  ...
  const [newdb, setNewdb] = useState(Moviedb)
  const Adding=(movie)=>setNewdb([...Moviedb,movie])
  ...
  const RoutedMovies=newdb.map(el=><Route path={`/movie/${el.title.split(" ").join("")}`}> <MovieCardExtended movie={el}/></Route>)
  return (
    ...
    <Router>
      ...
      <div className="addButton">
          ...
          <NavLink className="btn effect01" to="/"><span>Home</span></NavLink> 
          </div>
      ...
      <div className="addButton">
        ...
        <div onClick={()=>setPopUp(true)} className="container1">
          <a className="btn effect01"><span>add a movie</span></a>
        </div>
      
      </div>
      {/*add button*/}
      <div className="search">
      <input type="text" placeholder="Search by movie title" onChange={event => setSearch(event.target.value)} />
    ...
    C4.622,10.623,2.833,8.831,2.845,6.631L2.845,6.631z" />
      ...
      <Route exact path="/">
        <div className="flex">
        <MovieList Filtering={search} Rating={rating} db={newdb} />
        </div>
      </Route>
      {RoutedMovies}      
      </Switch>
      <div><AddForm trigger={popUp} toggle={setPopUp} adding={Adding}/></div>
    </Router>
    </>
  );
}
